...
resource "tls_private_key" "shiapp_server" {
    algorithm =  "RSA"
    rsa_bits  = 4096
}
resource "aws_key_pair" "shiapp_server_key" {
    key_name = var.keypair_name
    public_key = tls_private_key.shiapp_server.public_key_openssh
  
}

output "private_key" {
  value     = tls_private_key.shiapp_server.private_key_pem
  sensitive = true
}
